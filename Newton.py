...
class Newton:
    ...
    def newton_test(self):
        x_vals = [-1, 0, 1, 2]
        y_vals = [1, 1, 2, 0]
        coefficients = self.newton_div_diff(x_vals, y_vals)
    ...
    def get_coefficients(self, x_values, y_values):
        self.coefficients = self.newton_div_diff(x_values, y_values)
        ...
        for i in range(1, degree):
            for j in range(0, degree + i):
                if j == 0:
                    ...
                    if first:
                        temp_coeffs[j] = (
                            coeffs[j-1] * coeffs[j-2]) + temp_coeffs[j]
                        temp_coeffs2[j] = temp_coeffs[j]
                        first = False
                    else:
                        temp_coeffs2[j] = (
                            temp_coeffs[j-1] * coeffs[i]) + temp_coeffs2[j]
                else:
...
#nt = Newton()
#coefficients = nt.newton_div_diff(x_vals, y_vals)
